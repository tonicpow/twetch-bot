...
function initTwetch(){
    const twetch = new Twetch({clientIdentifier: process.env.clientIdentifier, 
...
const post = async (instance, content, reply, branch, filesURL, tweet, hide) => {
	try {
...
const main = async() => {
    ...
    while (true){
        ...
        await sleep(parseInt(process.env.refreshRate));
    }
}
main();